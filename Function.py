...
def pre_treatement_set3(output_size = 1, test_dataset_size = 10000, TRAIN_SET_NUM = 10, SEQUENCE_SIZE = 20, Ts_train = 0.001, predict_sample_num = 1, batchsize = 1, fs_resample = 44.1):
    ...
    nonlinear_ref, fs = load_wav('../dataset/nonstationary_noise/nonstationary_sound.wav')
    
    kind = 'cubic'
    fs_orig=19.98*10**3             # 19.98Khz
    size = int(len(nonlinear_ref) * fs_resample / fs_orig)
    
     # 一時保存用配列
    nonlinear_ref_temp = np.zeros([size,1])
    # アップサンプリング実行を一時保存用配列へ
    nonlinear_ref_temp = Myfunction.UPSAMPLING(signal=nonlinear_ref,fs_orig=fs_orig, fs_resample=fs_resample, KIND=kind)
    # この後使う配列へ代入
    nonlinear_ref = np.zeros([size,1])

    # f16, leopard, volvo, destroyerengine =f16_temp, leopard_temp, volvo_temp, destroyerengine_temp
    nonlinear_ref = nonlinear_ref_temp.copy()
    # 一次信号の変数に、参照信号の値を代入する(_pri = _ref)
    nonlinear_pri = nonlinear_ref_temp.copy()

    divide = int(nonlinear_pri.shape[0]*0.5)

    NOISE_train_ref = np.vstack([nonlinear_ref[:divide], nonlinear_ref[:divide], nonlinear_ref[:divide], nonlinear_ref[:divide]])
    NOISE_train_pri = np.vstack([nonlinear_pri[:divide], nonlinear_pri[:divide], nonlinear_pri[:divide], nonlinear_pri[:divide]])

    # ################################################################################################
    # (4, 5203786, 1)
    NOISE_test_ref = np.stack([nonlinear_ref[divide:], nonlinear_ref[divide:], nonlinear_ref[divide:], nonlinear_ref[divide:]], axis=0)
    NOISE_test_pri = np.stack([nonlinear_pri[divide:], nonlinear_pri[divide:], nonlinear_pri[divide:], nonlinear_pri[divide:]], axis=0)
